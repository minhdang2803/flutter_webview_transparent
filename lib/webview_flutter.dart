...
class NavigationRequest {
  NavigationRequest._({required this.url, this.isForMainFrame = true});
...
class _WebViewState extends State<WebView> {
  ...
  void _assertJavascriptChannelNamesAreUnique() {
    if (widget.javascriptChannels == null ||
        widget.javascriptChannels?.isEmpty == true) {
      return;
    }
...
WebSettings _clearUnchangedWebSettings(
  ...
  assert(currentValue?.userAgent.isPresent == true);
...
class _PlatformCallbacksHandler implements WebViewPlatformCallbacksHandler {
  ...
  FutureOr<bool> onNavigationRequest(
      {String url = "", bool isForMainFrame = false}) async {
...
class WebViewController {
  ...
  Future<String> currentUrl() async {
    return await _webViewPlatformController?.currentUrl() ?? "";
  }
  ...
  Future<bool> canGoBack() async {
    return await _webViewPlatformController?.canGoBack() ?? false;
  }
  ...
  Future<bool> canGoForward() async {
    return await _webViewPlatformController?.canGoForward() ?? false;
  }

  /// Goes back in the history of this WebView.
  ///
  /// If there is no back historay item this is a no-op.
  Future<void> goBack() async {
    return await _webViewPlatformController?.goBack();
  }
  ...
  Future<void> goForward() async {
    return await _webViewPlatformController?.goForward();
  }
  ...
  Future<void> _updateSettings(WebSettings newSettings) async {
  ...
  Future<String> evaluateJavascript(String javascriptString) async {
    ...
    return await _webViewPlatformController
  ...
  Future<String> getTitle() async {
    return await _webViewPlatformController?.getTitle() ?? "";
  }
  ...
  Future<void> scrollTo(int x, int y) async {
    return await _webViewPlatformController?.scrollTo(x, y);
  }
  ...
  Future<void> scrollBy(int x, int y) async {
    return await _webViewPlatformController?.scrollBy(x, y);
  }
  ...
  Future<int> getScrollX() async {
    return await _webViewPlatformController?.getScrollX() ?? 0;
  }
  ...
  Future<int> getScrollY() async {
    return await _webViewPlatformController?.getScrollY() ?? 0;
  }
}
